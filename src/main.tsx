...
function HomePage() {
  ...
  async function loadGithubStatus(isActive?: () => boolean) {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.ok ? "Unexpected API response" : data.error);
      }
      ...
      <section className="mx-auto w-full max-w-6xl">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <article className="rounded-3xl border border-white/12 bg-black/35 p-5 backdrop-blur-sm">
            ...
            <p className="text-sm leading-6 text-zinc-300">
              Vite exposes `/api/health` and `/api/github/status` during local
              development.
            </p>
          </article>
          ...
          <article className="rounded-3xl border border-white/12 bg-black/35 p-5 backdrop-blur-sm">
            <h2 className="mb-2 text-base font-medium">Next step</h2>
            <p className="text-sm leading-6 text-zinc-300">
              Build the PR browser, diff view, and review workflow on top.
            </p>
          </article>
        </div>
      </section>
